...
# 1 curve per sex, with individual random effect
...
bc_data <-  list(age = age2, length = length2, length2=length2,group = group2, 
                 ...
                 N = length(age2), G = length(unique(sex)))
...
sink('vb_2curve_sex_071323.jags')
cat(
  "model{
for(i in 1:N){
...
vbc <- jagsUI::jags(data=bc_data, inits=inits, parameters.to.save=params, model.file="vb_2curve_sex_071323.jags", 
...
save(vbc, file='vb_2curve_sex_071323.rda')

load('C:\\Users\\sarah.clements1\\Documents\\GitHub\\SMB_Fitness\\vb_2curve_sex_071323.rda')
...
linftable <- data.frame(Linf = c(LINF1, LINF2), 
                        Sex=c(rep("Female", length(LINF1)), rep("Male", length(LINF2))))
ktable <- data.frame(K = c(K1, K2), 
                     Sex=c(rep("Female", length(K1)), rep("Male", length(K2))))

linf_sex <-ggplot(linftable)+
...
# lines
lines(x=age_seq, y=curves[[1]]$mean, type='l', col='gray20', lwd=2) # female
lines(x=age_seq, y=curves[[2]]$mean, col='gray50', lwd=2) # male
# linf lines
