...
### STEP 2: Assess heterozygosity-fitness corrlation
In this step, I use the condition data and standardized multi-locus heterozygosity to assess the correlation between these variables, i.e., to determine if there is a generalized heterozygosity-fitness correlation (HFC)

##### Assess heterozygosity fitness correlation:
```{r}
load("data/hfc_data.Rda")
...
hfc_data <- hfc_data %>%
  mutate(condition_alive = 1000*(mass_dead/(tl_alive^3)))
  
ggplot(hfc_data, aes(x = h, y = condition)) +
  geom_point(position = position_jitter()) +
  geom_smooth(method = "lm")

ggplot(hfc_data, aes(x = smb, y = condition, color = ancestry_group)) +
  geom_point(position = position_jitter()) +
  geom_smooth(method = "lm")

ggplot(hfc_data, aes(x = ancestry_group, y = condition, color = ancestry_group)) +
  geom_boxplot() +
  geom_point(position = position_jitter(width = 0.3))


mod <- lm(condition ~ ancestry_group, data = hfc_data)
