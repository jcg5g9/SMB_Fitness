...
load("data/otolith_recounts/originals.Rda")
...
#### 2c: Calculate and plot coefficient of variation across ages for inter-observer error; run the Rmd chunk below.
In this step, we calculate the coefficient of variation (CV; standard deviation divided by the mean) across readers for each individual fish to determine whether there was a trend in the error associated with age estimation.

##### Calculate coefficient of variation across observers and ages:
```{r}
# Load original count data
load("data/otolith_recounts/originals.Rda")
...
# calculate coefficient of variation across readers for each fish
cv <- data.frame("cv" = apply(originals, 1, function(x) sd(x)/mean(x) * 100))

# column bind cv values with consensus age
age <- full_ancestry_data %>%
  dplyr::select(consensus_age)

# column bind cv values with consensus age
cv <- cbind(age, cv)

# Calculate mean cv values by age
cv_mean <- cv %>%
  group_by(consensus_age) %>%
  summarize(n = n(),
            mean_cv = mean(cv),
            sd_cv = sd(cv))

# save cv_mean values for plotting
save(cv_mean, file = "data/otolith_recounts/cv.Rda")
```
...
### STEP 1: Set "device type" in the package RFishBC (Ogle, 2022) so that the GUI will be readible by MAC OS (this will change depending on your operating system); run the Rmd chunk below.
...
jpeg("figures/bc_p_diffs.jpeg", width = 3000, height = 1500, res = 500)
...
jpeg("figures/bc_sex_betas.jpeg", width = 2000, height = 1500, res = 500)
...
# Plot sex beta coefficient 95% CIs
jpeg("figures/bc_stream_betas.jpeg", width = 2000, height = 1500, res = 500)
...
jpeg("figures/bc_predict.jpeg", width = 4600, height = 3900, res = 500)
...
# Select only posterior densities for stream- and sex-specific fixed effects
stream_sex_betas <- sensitivity_fixed %>%
  filter(parameter == "beta_linf[1]" |
           parameter == "beta_linf[2]" |
           parameter == "beta_k[1]" |
           parameter == "beta_k[2]" |
...
# Plot sex beta coefficient 95% CIs
jpeg("figures/sensitivity_sex_betas.jpeg", width = 1500, height = 1100, res = 300)
...
# Plot sex beta coefficient 95% CIs
jpeg("figures/sensitivity_stream_betas.jpeg", width = 1500, height = 1100, res = 300)
...
f_bs_plot <- ggplot() + 
  ...
  scale_fill_manual(values = c("grey","grey")) +
  scale_x_continuous(name = "Annulus (years)", expand = c(0,0), breaks = seq(1, 15, 1)) +
  scale_y_continuous(name = "Total length at capture (mm)", limits = c(0,700), breaks = seq(0, 700, 100)) +
  theme(axis.title = element_text(size = 15)) + 
  theme(axis.text = element_text(size = 15)) +
  theme(panel.border = element_rect(colour = "black", fill = NA)) +
  theme(plot.margin = margin(15,15,15,15))
...
# Plot all von bertalanffy curves
jpeg("figures/sensitivity_predict.jpeg", width = 3000, height = 2400, res = 300)
