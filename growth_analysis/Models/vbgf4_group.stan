//  VBGF with sex/river effects saved as vbgf4_group.stan
...
transformed parameters {
  ...
  // Predicted length
  for(i in 1:Nobs){
    length_hat[i] = linf_ind[id[i]] * exp(eta_ind[id[i],1] + eta_lineage[group[i], 1]) * (1-exp(-k_ind[id[i]] * exp(eta_ind[id[i],2]  + eta_lineage[group[i], 2]) * (age[i] - (t0_ind[id[i]] + eta_ind[id[i],3] + eta_lineage[group[i], 3]))));
  }
}
model {
  // Priors
