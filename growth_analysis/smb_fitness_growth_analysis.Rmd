...
full_bc_data <- full_bc_data %>%
  mutate(ancestry_mod = factor(ancestry_mod))

full_bc_data <- full_bc_data %>%
  mutate(river = factor(river))

com <- bc_tl ~ Linf*(1-exp(-K*(annulus-t0)))
com_starts <- vbStarts(bc_tl ~ annulus, data = full_bc_data)
fit_com <- nls(com,
               data = full_bc_data,
               start = com_starts)


gen <- bc_tl ~ Linf[river]*(1-exp(-K[river]*(annulus-t0[river])))
gen_starts <- lapply(com_starts, rep, 2)
fit_gen <- nls(gen,
               data = full_bc_data,
               start = gen_starts)

L_com <- bc_tl ~ Linf*(1-exp(-K[river]*(annulus-t0[river])))
L_com_starts <- mapply(rep, com_starts, c(1,2,2))
fit_L_com <- nls(L_com,
               data = full_bc_data,
               start = L_com_starts)

K_com <- bc_tl ~ Linf[river]*(1-exp(-K*(annulus-t0[river])))
K_com_starts <- mapply(rep, com_starts, c(2,1,2))
fit_K_com <- nls(K_com,
               data = full_bc_data,
               start = K_com_starts)

t_com <- bc_tl ~ Linf[river]*(1-exp(-K[river]*(annulus-t0)))
t_com_starts <-mapply(rep, com_starts, c(2,2,1))
fit_t_com <- nls(t_com,
               data = full_bc_data,
               start = t_com_starts)

K_L_com <- bc_tl ~ Linf*(1-exp(-K*(annulus-t0[river])))
K_L_com_starts <- mapply(rep, com_starts, c(1,1,2))
fit_K_L_com <- nls(K_L_com,
               data = full_bc_data,
               start = K_L_com_starts)

K_t_com <- bc_tl ~ Linf[river]*(1-exp(-K*(annulus-t0)))
K_t_com_starts <- mapply(rep, com_starts, c(2,1,1))
fit_K_t_com <- nls(K_t_com,
               data = full_bc_data,
               start = K_t_com_starts)

L_t_com <- bc_tl ~ Linf*(1-exp(-K[river]*(annulus-t0)))
