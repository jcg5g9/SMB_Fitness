...
ggplot(full_bc_data, aes(x = residuals)) +
  geom_histogram()

full_bc_data_four <- full_bc_data %>%
  filter(annulus > 3)

ggplot(full_bc_data_four, aes(x = ancestry_group, y = residuals)) +
  geom_boxplot() +
  geom_point()

mod <- lm(residuals ~ ancestry_group, full_bc_data_four)
...
full_bc_data_global <- full_bc_data_global %>%
...
ggplot() + 
  ...
  geom_point(data = full_bc_data_global, aes(x = annulus, y = bc_tl, fill = ancestry_group), show.legend = F, size = 3, pch = 21, position = position_jitter(width = 0.2)) + 
