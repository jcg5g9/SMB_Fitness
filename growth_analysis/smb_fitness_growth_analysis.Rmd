...
#### 3c: Plot 95% credible intervals for posterior differences of L∞, k, and a0 parameters; run the Rmd chunk below.
...
#### 3c: Plot 95% credible intervals for beta coefficient posterior densities of L∞, k, and a0 parameters for sex and stream; run the Rmd chunk below.
...
#### 3d: Plot predicted length-at-age using calculated parameter values; run the Rmd chunk below.
...
# Get numeric vectors to hold means and confidence interval estimates from predictions
...
f_bs_smb_mean_length$ancestry <- factor(rep("SMB", times = 15))
f_bs_smb_mean_length$sex <- factor(rep("Female", times = 15))
f_bs_smb_mean_length$stream <- factor(rep("Big Sugar Creek", times = 15))
...
plot(y = length , x = age, 
     ...
     xlim = c(0, bc_model_data$Nages), ylim = c(0, 500))

# - Plot median curve
lines(1:bc_model_data$Nages, apply(draws[,grepl("pred_length\\[1,",colnames(draws))], 2, median), col = 1, lty = 1, lwd = 4) # Global
