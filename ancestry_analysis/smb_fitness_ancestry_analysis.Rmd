...
## PHASE 2: HETEROZYGOSITY INFERENCE
In this phase of the analysis, we compute standardized multilocus heterozygosity for each individual, which is the individual heterozygosity relative to the global mean heterozygosity value. 

### STEP 1: Read in raw genotype data and convert to input format for inbreedR package.
In this step, we convert the raw genotype data into the correct input format for the inbreedR package (which is used to assess heterozygosity fitness correlation)

#### 1a: Read in and format raw genotype data; run the Rmd chunk below.

##### Read in and format data for inbreedR:
```{r}
# Load genotype data
load("../filtering_analysis/data/processed_raw_data/genotype_data.Rda")

# Omit sample id column, only retaining raw genotype data
genotype_data <- genotype_data[,-c(1)]

# generate dataframe for inbreedR; convert sample id column into row names
ibr <- column_to_rownames(genotype_data, "structure_number")

# Convert data to inbreedR format
ibr <- convert_raw(ibr)

# Check dataset in inbreedR
check_data(ibr)

# Save inbreedR data object
save(ibr, file = "data/inbreedr_data/ibr.Rda")
```

### STEP 2: Calculate g2 statistic and heterozygosity values (significant g2 indicates that heterozygosity values are correlated such that they are indicative of genome-wide heterozygosity)

#### 2a: Calculate g2 statistic; run the Rmd chunk below:

##### Calculate g2 statistic:
```{r}
# Load ibr data
load("data/inbreedr_data/ibr.Rda")

# Calculate g2 statistic in inbreedR
g2 <- g2_microsats(ibr,
                   nperm = 1000, 
                   nboot = 1000, 
                   CI = 0.95)
...
p(g2) = 0.019 (significant); there is sufficient power to detect genome-wide heterozygosity

#### 2b: Calculate standardized multilocus heterozygosity; run the Rmd chunk below:
...
# Compute standardized multi-locus heterozygosity
h <- sMLH(ibr)

# Convert to data frame
h <- data.frame(h)

# Convert rownames (sample id) back to column
h <- rownames_to_column(h, "structure_number")

# Convert characters to factors for sample id column
h <- h %>%
  mutate(structure_number = factor(structure_number))

# Save heterozygosity data object
save(h, file = "data/inbreedr_data/h.Rda")
```

### ----------------------- END OF PHASE 2: HETEROZYGOSITY INFERENCE ----------------------- ###

## PHASE 3: ANCESTRY AND HETEROZYGOSITY VISUALIZATION
In this phase of analysis, we visualize ancestry coefficients and heterozygosity across the full sample set.
...
# Load processed heterozygosity data
load("data/inbreedr_data/h.Rda")

# Separate out and create data frame for Neosho Bass samples only
q_nb <- q[c(1:116),]
h_nb <- h[c(1:116),]

# Merge ancestry and heterozygosity data
ancestry_nb <- merge(q_nb, 
                     h_nb,
                     by = "structure_number")
...
### STEP 3: Summarize ancestry coefficients and heterozygosity for all individuals. 
