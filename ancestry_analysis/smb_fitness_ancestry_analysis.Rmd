...
# Calculate mean ancestry proportion in the SMB reference samples
full_ancestry_data %>%
  filter(population == "reference") %>%
  summarize(mean_smb = mean(smb))

# Calculate mean ancestry proportions by species (NB and SMB) within the NB native range
full_ancestry_data %>%
  filter(population == "sample") %>%
  summarize(mean_smb = mean(smb),
            mean_nb = mean(nb))
...
# Summarize SMB ancestry range in reference SMB samples 
full_ancestry_data %>%
  filter(population == "reference") %>%
  arrange(desc(smb))
...
<b>NB</sub> = 0.6176129 </b><br>
<b>SMB</sub> = 0.3823871 </b><br>

## Totals by ancestry group 
<b><i>N</i><sub>Neosho_Bass</sub> = 46 </b><br>
<i>N</i><sub>Smallmouth Bass</sub> = 14 <br>
