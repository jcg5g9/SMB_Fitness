...
save(data, file = "data/ac_data.Rda")
```
...
#### 3a: Summarize mean and 95 credible intervals for all model coefficients; run the Rmd chunk below:
...
length(which(ac_fit$beta1<0))/length(ac_fit$beta1) # 0.96725
length(which(ac_fit$beta2>0))/length(ac_fit$beta2) # 0.986
...
#### 3b: Plot 95% credible intervals for each parameter coefficient; run the Rmd chunk below.
...
#### 3c: Plot predicted condition values across ancestry along with raw (converted) data; run the Rmd chunk below.
...
# Get sequence of smb values to predict
smb <- seq(0, 1, 0.1)
...
pdf("figures/ac_predict.pdf", width = 6, height = 5)

ggplot() + 
  geom_point(data = condition_data, aes(x = smb, y = condition_mg_cm), fill = "grey", show.legend = F, size = 3, pch = 21) + 
  geom_ribbon(data = predicted_condition, aes(x = smb, ymin = lci, ymax = uci, fill = "grey", alpha = 0.2), show.legend = F) +
  geom_line(data = predicted_condition, aes(x = smb, y = condition, color = "black"), size = 1, show.legend = F) +
  theme_set(theme_cowplot(12)) +
  scale_fill_manual(values = c("grey")) +
  scale_color_manual(values = c("black")) +
  scale_x_continuous(name = "Proportion of SMB ancestry (%)", expand = c(0,0), limits = c(0,1), labels = number_format(accuracy = 0.001)) +
