...
condition_data_4 <- condition_data %>%
  filter(consensus_age <= 4) %>%
  mutate(condition_alive = mass_dead/(tl_alive^3))

ggplot(condition_data_4, aes(x = h, y = condition_alive)) +
  geom_point() +
  geom_smooth(method = "lm")

summary(lm(condition ~ h, data = condition_data_4))
