...
There was a discrepancy in the number of sample ids present in the metadata for the sample set (<i>N</i> = 139) and the number present in the phenotype data (<i>N</i> = 139). We therefore conducted a direct comparison of the two columns to see where they differed and to identify any sample ids without corresponding data in both datasets.
...
## PHASE 2: DATA SUMMARIZATION AND ENUMERATION
In this phase of the analysis, we are fully enumerating the genotype and phenotype datasets so that we know how many samples belong to each group of interest for the final manuscript. 

### STEP 1: Summarize the full genotype and phenotype datasets
In this step, we are summarizing the full genotype and phenotype datasets together (merged), with appended metadata, to enumerate samples by groups of interest. We also ensure that both datasets contain the same samples.

#### 1a. Read in and summarize the full genotype data; run the Rmd chunk below:
...
## Verify that the two datasets contain the same sample ids

# Samples in genotype data but not in phenotype data
full_genotype_data %>% 
  filter(!full_genotype_data$sample_id %in% full_phenotype_data$sample_id)

# Samples in phenotype data but not in genotype data
full_phenotype_data %>% 
  filter(!full_phenotype_data$sample_id %in% full_genotype_data$sample_id)




# Merge genotype and phenotype data by sample id
full_data <- merge(full_genotype_data, full_phenotype_data, by = "sample_id")

# Summarize genotype data by population (sample vs. reference)
full_genotype_data %>% 
  group_by(population) %>%
  count()

# Summarize genotype data by river within population
full_genotype_data %>% 
  group_by(population, river) %>%
  count()
```
